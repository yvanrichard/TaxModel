...
lm.ins <- lmer(lDD~lBDT+(1|Species)+(1|Genus)+(1|Family)+(1|Order),data=insects_ss)

plot(fitted(lm.ins),insects_ss$lDD[!is.na(insects_ss$lDD)])
abline(a=0,b=1)

confint.merMod(lm.ins,method='Wald')
confint.merMod(lm.ins)


# reproduce results
vcv <- as.data.frame(lme4::VarCorr(lm.ins))
vcv$vcov/sum(vcv$vcov)

lm.ins.pred <- predict(lm.ins,insects[is.na(insects$lDD)&!is.na(insects$BDT),],allow.new.levels = T)
...
plot(insects_ss$lDD,lm.cv.spred[,1])
...
lm.pred <- merTools::predictInterval(lm.ins,newdata=ii)
reSims <- merTools::REsim(lm.ins, n.sims = 100)
