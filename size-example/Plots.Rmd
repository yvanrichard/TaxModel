...
ggplot(msd) + 
  facet_wrap(~Factor)+
  geom_point(aes(x=Var2*5, y=means, col=model), size=4, position=position_dodge(width=dw)) + 
 
  geom_linerange(aes(x=Var2*5, y=means,ymin=q1,ymax=q3,col=model),size=1, position=position_dodge(width=dw)) + 
  geom_linerange(aes(x=Var2*5, y=means,ymin=q11,ymax=q33,col=model),size=2, position=position_dodge(width=dw)) + 
  theme_classic()+
   geom_point(aes(x=Var2*5, y=Mass), size=4, position=position_dodge(width=dw)) +
  #coord_flip() #+ 
  xlab('Prediction') + 
  ylab('Test point')
