...
msd <- msd %>% 
  mutate(Factor = do.call('rbind',strsplit(L2,'sd.'))[,2]) %>%
  group_by(L1,Factor) %>%
...
sds <- list()
sds[[1]] <- JM_rfx$BUGSoutput$sims.list[grepl('mu_pred',names(JM_rfx$BUGSoutput$sims.list))]
sds[[2]] <- JM_rfx_h$BUGSoutput$sims.list[grepl('mu_pred',names(JM_rfx_h$BUGSoutput$sims.list))]
#sds[[3]] <- JM$BUGSoutput$sims.list[grepl('sd',names(JM$BUGSoutput$sims.list))]
...
msd$Factor <- factor(msd$Factor,levels = c('Species','Genus','Family','Order'))
msd$log10Mass <- sizes$lmass[pred]
msd <- msd %>% group_by(model,Factor) %>% arrange(log10Mass) %>% mutate(Var2 = 1:n())
...
ggplot(msd) + 
  ...
  theme_classic()+
   geom_point(aes(x=Var2*3, y=log10Mass), size=4, position=position_dodge(width=dw)) 
  #coord_flip() #+ 
  #ylab(expression(Finite~population~variance~(log[10]~PPMR))) + 
  #xlab('Effect')
